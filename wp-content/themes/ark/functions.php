...
define('FF_ARK_THEME_VERSION', '1.44.0');
...
if( !function_exists('ff_speed_move_jquery_to_footer') ) {
	function ff_speed_move_jquery_to_footer() {
		if( !class_exists('ffThemeOptions') ) {
			return;
		}

		$moveJQueryToFooter = ffThemeOptions::getQuery()->getWithoutComparationDefault('theme_options speeding move-jquery-to-footer', 0);

		if( $moveJQueryToFooter ) {
	...
	add_action( 'wp_enqueue_scripts', 'ff_speed_move_jquery_to_footer' );
}

if( !function_exists('ff_remove_emoji_scripts') ) {
	function ff_remove_emoji_scripts() {
		if( !class_exists('ffThemeOptions') ) {
			return;
		}

		$removeEmojiScripts = ffThemeOptions::getQuery()->getWithoutComparationDefault('theme_options speeding remove-emoji-detection-scripts', 0);

		if( $removeEmojiScripts ) {
			remove_action('wp_head', 'print_emoji_detection_script', 7);
			remove_action('wp_print_styles', 'print_emoji_styles');
		}
	}
	add_action('wp', 'ff_remove_emoji_scripts');
}
